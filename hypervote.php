	...
	do {
		foreach ( $data_targ as $key => $d ) {
			try {
				...
				if ( $is_gf_first ) {
					$climate->info( $number . ' Get followers from @' . $d['username'] . ' Successfully.' );
					$is_gf_first = 0;
				} else {
					$climate->info( 'Next ' . $number . ' followers with valid stories of @' . $d['username'] . ' Collected. Total: ' . number_format( $data_targ[ $key ]['users_count'], 0, '.', ' ' ) . ' Followers from @' . $d['username'] . ' parsed.' );
				}
				echo "="; for($x = 0; $x <= 4; $x++){ echo "============="; sleep(2); } echo "\n\n";
